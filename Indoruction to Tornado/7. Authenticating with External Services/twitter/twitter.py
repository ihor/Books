...
class TwitterHandler(tornado.web.RequestHandler, tornado.auth.TwitterMixin):
	...
	def get(self):
		...
		if self.get_argument('oauth_token', None):
			...
			self.twitter_request('/users/show', access_token=accessToken, user_id=userID, callback=self.async_callback(self._twitter_on_user))
			return
...
class Application(tornado.web.Application):
	def __init__(self):
		handlers = [(r'/', TwitterHandler), (r'/logout', LogoutHandler)]
		settings = {
